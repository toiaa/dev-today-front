...
import ContentNavbar from "../shared/header/ContentNavbar";
...
const ProfilePage = ({ user, type, isOwner, posts }: ProfilePageProps) => {
  return (
    <div className="flex flex-col gap-5 lg:flex-row">
      <aside className="relative flex shrink-0 flex-col items-start gap-5 border-0 lg:sticky lg:left-0 lg:top-0 lg:min-w-[210px]">
        <ProfileCard user={user} isOwner={isOwner} />
      </aside>
      <div className="flex size-full flex-col gap-5 ">
        <ContentNavbar type={type} />
        {posts === null ? (
          <div>
            <p className="text-center text-3xl">No posts found</p>
          </div>
        ) : (
          <InfiniteScroll type={type} user={user} initialContent={posts} />
        )}
      </div>
